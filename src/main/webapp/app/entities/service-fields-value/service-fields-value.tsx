...
export interface IServiceFieldsValueState extends IPaginationBaseState {
  dropdownOpenTop: boolean;
  dropdownOpenBottom: boolean;
  itemsPerPage: number;
}

export class ServiceFieldsValue extends React.Component<IServiceFieldsValueProps, IServiceFieldsValueState> {
  constructor(props) {
    super(props);
  ...
  render() {
    const { serviceFieldsValueList, match, totalItems } = this.props;
    return (
      <div>
...
const mapStateToProps = ({ serviceFieldsValue }: IRootState) => ({
  ...
  totalItems: serviceFieldsValue.totalItems
});
