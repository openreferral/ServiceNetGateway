...
export interface IFieldExclusionState extends IPaginationBaseState {
  dropdownOpenTop: boolean;
  dropdownOpenBottom: boolean;
  itemsPerPage: number;
}

export class FieldExclusion extends React.Component<IFieldExclusionProps, IFieldExclusionState> {
  constructor(props) {
    super(props);
  ...
  render() {
    const { fieldExclusionList, match, totalItems } = this.props;
    return (
      <div>
...
const mapStateToProps = ({ fieldExclusion }: IRootState) => ({
  ...
  totalItems: fieldExclusion.totalItems
});
